...
function canAccessTicket(t: any, u: { username: string; rol: string }) {
  ...
  if (isSuperv) {
    ...
    return (t.accesoGrantado || []).includes(u.username)
  }
  return false
}
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canAccessTicket(t, u)) return forbidden()
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!Array.isArray(next.accesoGrantado)) next.accesoGrantado = []
  ...
  // Grant / revoke acceso a supervisory para categorías restringidas (solo command_staff)
  if (body.grantAcceso && isCS) {
    const username = String(body.grantAcceso).trim()
    if (username && !next.accesoGrantado.includes(username)) next.accesoGrantado.push(username)
  }
  if (body.revokeAcceso && isCS) {
    const username = String(body.revokeAcceso).trim()
    next.accesoGrantado = next.accesoGrantado.filter((x: string) => x !== username)
  }

  if (body.comentario) {
    const comentario = String(body.comentario).trim()
    if (comentario.length > 2000) return NextResponse.json({ error: 'Comentario demasiado largo (máx 2000)' }, { status: 400 })
  ...
  try { await persistTicket(next) }
  catch { return NextResponse.json({ error: 'No se pudo persistir el ticket. Reintenta.' }, { status: 503 }) }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { await deleteTicketById(id) }
  catch { return NextResponse.json({ error: 'No se pudo eliminar el ticket. Reintenta.' }, { status: 503 }) }
